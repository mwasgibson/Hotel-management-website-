    ...
    db.query(
        ...
        (err, rows) => {
            ...
            rows.forEach((row) => {
                ...
                try {
                    content[row.page][row.section][row.content_key] = parseValue(row.content_value, row.content_type);
                } catch {
                    content[row.page][row.section][row.content_key] = row.content_value;
                }
            });

            res.json({ content, records: rows });
        }
    );
};
    ...
    db.query(
        ...
        (err, rows) => {
            ...
            res.json(rows.map((row) => ({
                ...row,
                value: (() => { try { return parseValue(row.content_value, row.content_type); } catch { return row.content_value; } })(),
            })));
        }
    );
};
...
exports.upsertContent = (req, res) => {
    ...
    if (!page || !section || !key) {
        return res.status(400).json({ error: 'page, section, and key are required' });
    }
    if (!VALID_TYPES.includes(type)) {
        return res.status(400).json({ error: 'type must be text, number, boolean, or json' });
    }

    let serialized;
    try {
        serialized = serializeValue(value, type);
        if (type === 'json') JSON.parse(serialized);
    } catch {
        return res.status(400).json({ error: 'Invalid value for the selected content type' });
    }
    ...
    db.query(sql, [page, section, key, serialized, type], (err, result) => {
        ...
        res.json({ message: 'Content saved', id: result.insertId || null });
    });
};
